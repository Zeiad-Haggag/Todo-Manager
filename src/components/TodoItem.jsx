  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <button
          className={`shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-200 m-0.5 ${
            todo.completed
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2 text-xs  text-gray-600">
            <div className="flex items-center gap-1 ">
              <Calendar size={12} />
              <span> Created {formattedDate(todo.createdAt)}</span>
            </div>
            <span> Update {formattedDate(todo.updatedAt)}</span>
          </div>
        </div>
